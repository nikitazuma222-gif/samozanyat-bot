...
# Gemini API key — получить бесплатно: https://aistudio.google.com/app/apikey
_raw_gemini = os.getenv("GEMINI_KEY", "")
# Railway иногда сохраняет как "GEMINI_KEY=AIza..." — исправляем автоматически
GEMINI_KEY = _raw_gemini.split("=", 1)[-1] if "=" in _raw_gemini else _raw_gemini

VISION_PROMPT = """Ты — система распознавания деловых документов на русском языке.
Внимательно прочитай ВЕСЬ текст на изображении и извлеки данные.

Верни ТОЛЬКО JSON без пояснений, без markdown, без лишних символов:
{"executor": "...", "client": "...", "amount": "...", "date": "...", "work": "..."}

Правила извлечения:
- executor: ФИО исполнителя/самозанятого (кто выполнял работу, подписывает как исполнитель)
- client: название организации-заказчика или ФИО заказчика (кто платит, кто заказывает)
- amount: ТОЛЬКО цифры суммы итого (например "32000"), без слова "рублей", без пробелов
- date: дата документа в формате ДД.ММ.ГГГГ (например "17.01.2025")
- work: краткое описание работ/услуг (1-2 предложения, что именно сделано)
- Если поле не найдено — пустая строка ""
- НЕ путай исполнителя и заказчика — исполнитель это тот кто делает работу"""

async def _parse_vision_response(text: str) -> dict:
    """Парсит JSON из ответа Vision AI"""
    try:
        start = text.find('{')
        end   = text.rfind('}') + 1
        ...
        logger.warning(f"JSON парсинг ошибка: {e}, текст: {text[:200]}")
    return {}

async def vision_extract_gemini(image_b64: str, mime: str = "image/jpeg") -> dict:
    """Распознавание через Google Gemini 1.5 Flash (бесплатный, самый точный)"""
    if not GEMINI_KEY:
        logger.info("GEMINI_KEY не задан — пропускаем Gemini")
        return {}
    try:
        async with httpx.AsyncClient(timeout=60) as c:
            ...
            result = await _parse_vision_response(text)
            if result:
                logger.info(f"Gemini Vision OK: {result}")
            return result
    except Exception as e:
        logger.warning(f"Gemini Vision ошибка: {e}")
        return {}

async def vision_extract_openrouter(image_b64: str, mime: str = "image/jpeg") -> dict:
    """Fallback — OpenRouter Vision модели (актуальные на 2026)"""
    models = [
        ...
        try:
            async with httpx.AsyncClient(timeout=60) as c:
                ...
                if "choices" not in data:
                    logger.warning(f"OpenRouter {model} нет choices: {data.get('error','')}")
                    continue
                text = data["choices"][0]["message"]["content"]
                result = await _parse_vision_response(text)
                if result:
                    logger.info(f"OpenRouter {model} OK: {result}")
                    return result
        except Exception as e:
...
async def vision_extract(image_b64: str, mime: str = "image/jpeg") -> dict:
    """Главная функция: Gemini (если ключ есть) → OpenRouter"""
    # Gemini работает с ключами AIzaSy (старый) и AQ. (новый формат Google AI Studio)
    if GEMINI_KEY and (GEMINI_KEY.startswith("AIza") or GEMINI_KEY.startswith("AQ.")):
        result = await vision_extract_gemini(image_b64, mime)
        if result:
            return result
        logger.info("Gemini не дал результат — пробуем OpenRouter")
    else:
        logger.info("Gemini ключ не задан или невалидный — используем OpenRouter")
    return await vision_extract_openrouter(image_b64, mime)
...
async def scan_receive_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Удаляем промежуточное сообщение и отправляем новое (edit_text не поддерживает ReplyKeyboard)
        try:
            await msg.delete()
        except Exception:
            pass
